...
def verify_blocking_ground_truth(dataset_name, block_df, objectify=False):
    ...
    print("Size of candset ="+" "+str(len(block_df)))
    print("Totally missed:"+" "+str(num_duplicates_missed) + " out of "+" "+str(total_duplicates))


def block_fodors_zagats(A, B):
    ob = em.OverlapBlocker()
    # No misses
    C = ob.block_tables(A, B, 'name', 'name', l_output_attrs=['name', 'addr', 'city', 'phone'],
...
def block_amazon_googleproducts(A, B):
    ...
    C = ob.block_tables(A, B, "title", "title", word_level=True, overlap_size=1,
                        ...
                        r_output_attrs=["title", "description", "manufacturer", "price"], show_progress=True,
                        allow_missing=True)
    return C
...
def block_abt_buy(A, B):
    ...
    C = ob.block_tables(A, B, "name", "name", word_level=True, overlap_size=1,
                        ...
                        r_output_attrs=["name", "description", "price"], show_progress=True, allow_missing=True)
    return C
...
def block_walmart_amazon(A, B):
    ...
    attributes = ['brand', 'groupname', 'title', 'price', 'shortdescr', 'longdescr', 'imageurl', 'modelno',
                  'shipweight', 'dimensions']
    C2 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=3, l_output_attrs=attributes,
...
def block_dblp_scholar(A, B):
    ...
    C2 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=4, show_progress=True,
