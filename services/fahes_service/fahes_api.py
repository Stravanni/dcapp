...
'''
Header formatting:
Some modules (e.g., ImputeDB) has some limitations supporting attribute names,
so here we format them (e.g., replacing "-' with "_"). 
'''
def format_headers(dir_in):
    files_in = os.listdir(dir_in)
    files_in = list(filter(lambda x: len(x.split(".csv"))>1,files_in)) # only .CSV supported
    ...
    for file in files_in:
        ...
        with open(file_in_path) as f:
            ...
            to_file = open(file_in_path,mode="w")
            to_file.write(line)
...
def transformSingleFile(file_in_path, metadata_file_path):
    table = pd.read_csv(file_in_path, encoding='utf-8',engine='python')

    statinfo = os.stat(metadata_file_path)
    if (statinfo.st_size == 0):  ## if == 0 : do not read
        return table

    df_dmv = pd.read_csv(metadata_file_path, encoding='utf-8', engine='python')
